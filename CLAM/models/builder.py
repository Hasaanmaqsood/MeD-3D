...
def get_encoder(model_name, target_img_size=224):
    ...
    if model_name in ['resnet50_trunc', 'resnet50']:
        # ✅ Support both 'resnet50_trunc' and 'resnet50'
        print(f'✅ Using TimmCNNEncoder for {model_name}')
        model = TimmCNNEncoder(model_name='resnet50')  # Ensure it's correctly mapped to timm
        ...
        raise NotImplementedError(f'❌ Model {model_name} not implemented in get_encoder()')

    print(f"✅ Model Loaded: {model}")
    
    # Ensure mean and std exist in constants for preprocessing
    # Ensure 'resnet50' exists in MODEL2CONSTANTS before accessing it
    if model_name not in MODEL2CONSTANTS:
        print(f"⚠️ Warning: Model name '{model_name}' not found in MODEL2CONSTANTS. Using default values.")
        constants = {'mean': [0.485, 0.456, 0.406], 'std': [0.229, 0.224, 0.225]}  # Default ImageNet values
    else:
