...
if __name__ == '__main__':
    # Initialize a dictionary to store debugging information
    debug_info = {
        "total_slides_found": 0,
        "slides_to_process": [],
        "process_column_values": None,
        "slides_processed": [],
        "slides_skipped": [],
        "patch_counts": {},
        "heatmap_generation_status": {}
    }
    ...
    if data_args.process_list is None:
        ...
        slides = df['slide_id'].tolist()  # Extract slide IDs from the process list

    # Store total slides found
    debug_info["total_slides_found"] = len(slides)
    debug_info["slides_to_process"] = slides

    # Initialize the DataFrame
    df = initialize_df(slides, def_seg_params, def_filter_params, def_vis_params, def_patch_params, use_heatmap_args=False)

    # Force all slides to be processed
    df['process'] = 1

    # Store process column values
    debug_info["process_column_values"] = df['process'].value_counts()
    ...
    # Store number of slides to process
    debug_info["slides_to_process_count"] = len(process_stack)
    ...
    print(f"🔍 Loading encoder model: {encoder_args.model_name}")
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        # Store slide ID being processed
        debug_info["slides_processed"].append(slide_id)
        ...
        if not os.path.isfile(h5_path):
            print("DEBUG: h5 file not found, computing features from patches")
        ...
        if not os.path.isfile(features_path):
            print("DEBUG: pt file not found, converting h5 to pt")
        ...
        # Store number of patches detected
        debug_info["patch_counts"][slide_id] = len(features)
        ...
        if not os.path.isfile(block_map_save_path): 
            print("DEBUG: block map not found, creating new one")
            file = h5py.File(h5_path, "r")
            coords = file['coords'][:]
            file.close()
        ...
        if os.path.isfile(os.path.join(r_slide_save_dir, heatmap_save_name)):
            print("DEBUG: heatmap already exists, skipping")
            debug_info["heatmap_generation_status"][slide_id] = "Heatmap skipped"
        else:
            print("DEBUG: creating new heatmap")
            ...
            del heatmap
            debug_info["heatmap_generation_status"][slide_id] = "Heatmap generated"
        ...
        if heatmap_args.save_orig:
            ...
            if os.path.isfile(os.path.join(p_slide_save_dir, heatmap_save_name)):
                print("DEBUG: original heatmap already exists, skipping")
                pass
            else:
                print("DEBUG: creating original heatmap")
    ...
    # Print debugging information at the end
    print("\n\n=== Debugging Information ===")
    print(f"Total slides found: {debug_info['total_slides_found']}")
    print(f"Slides to process: {debug_info['slides_to_process']}")
    print(f"Process column values: {debug_info['process_column_values']}")
    print(f"Number of slides to process: {debug_info['slides_to_process_count']}")
    print(f"Slides processed: {debug_info['slides_processed']}")
    print(f"Patch counts per slide: {debug_info['patch_counts']}")
    print(f"Heatmap generation status: {debug_info['heatmap_generation_status']}")