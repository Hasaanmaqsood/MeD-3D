            ...
            if attn_save_path is not None:
                ...
                if ref_scores is not None:
                    for score_idx in range(len(A)):
                        ...
                        print(f"🔍 Debug: ref_scores.shape = {ref_scores.shape}")
                        print(f"⚠️ Warning: score_idx is an integer: {score_idx}")
                        
                        percentile_value = np.mean(score2percentile(A[score_idx], ref_scores))

                
                        print(f"🔍 Debug: percentile_value = {percentile_value}, type: {type(percentile_value)}")

                        A[score_idx] = np.float32(percentile_value)  # Ensure it's a single float

                
                        print(f"✅ Debug: Successfully assigned percentile {percentile_value} to A[{score_idx}]")
