...
def collate_MIL(batch):
    """
    Custom collate function for MIL datasets.
    The batch is a list of tuples (features_tensor, label).
    """
    batch = [item for item in batch if item is not None]  # Remove None values

    if len(batch) == 0:
        return None  # Ensure we don’t pass an empty batch

    # Unzip batch into two lists: one for data, one for labels
    data_list, label_list = zip(*batch) 

    # Convert data_list to a PyTorch tensor
    data = torch.cat([item.unsqueeze(0) if item.dim() == 1 else item for item in data_list], dim=0)

    # Convert label_list to a PyTorch tensor
    labels = torch.tensor(label_list, dtype=torch.long)  # Ensure labels are a tensor
